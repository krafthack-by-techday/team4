...
$.extend(Datepicker.prototype, {
	...
	_showDatepicker: function(input) {
		...
		if (!inst.inline) {
			...
			inst.dpDiv.zIndex($(input).zIndex() + 120);
					...
					this._on( this.document, {
						focusin: function( event ) {
							if ( !$( event.target ).closest(".ui-dialog").length
                                // Emfip fix by Petr Chudoba (TPI-936)
                                // data is sometimes null, can't determinate the right scenario
                                 && $(".ui-dialog:visible:last .ui-dialog-content").data("ui-dialog") != null)
                            {
								event.preventDefault();
